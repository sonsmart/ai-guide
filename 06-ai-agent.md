...
> **Reflexion 解决了 ReAct 的什么问题：** ReAct 每次失败都从零开始重试，没有跨任务的记忆积累。Reflexion 的关键是把反思结果**存入长期记忆**，下次执行类似任务时能调取经验避免同样错误——是从失败中学习，而不只是"回顾是否符合预期"。
...
|------|-----------|------|-------------------|
| **搜索方式** | 线性重试 | 树搜索（MCTS/蒙特卡洛树搜索） | 生成-评估循环 |
...
**记忆类型来源：借鉴认知心理学（来自 Lilian Weng 的经典 Agent 文章）**

| 类型 | 对应 AI | 存在哪 | 说明 |
|------|---------|--------|------|
| **感知记忆（Sensory）** | 原始输入的 embedding | 极短暂，处理完即丢 | 相当于感官输入瞬间留存 |
| **短期/工作记忆（Short-term）** | Context Window 内的内容 | 内存 | 有限容量，会话内有效 |
| **长期 - 情景记忆（Episodic）** | 带时间戳的具体事件 | 向量数据库 | "什么时候发生了什么事" |
| **长期 - 语义记忆（Semantic）** | 抽象事实和知识 | 向量数据库 / 知识图谱 | "知道某个事实" |
| **长期 - 程序性记忆（Procedural）** | 模型权重本身、LoRA 微调 | 模型参数 | 技能和行为模式，无意识自动执行 |

情景 vs 语义的区别：语义记忆是"知道某个事实"（结论），情景记忆是"记得某件事是什么时候在什么情境下发生的"（带上下文的经历）。

程序性记忆是最特殊的一种——不存在外部数据库，而是固化在模型权重里，微调就是在修改程序性记忆。

长期记忆存储位置：取决于实现，可以是云端（绑定账号，跨设备同步）或本地（本机向量数据库，不上传）。
...
> **常见误解：** Copilot 不只限于"改代码"，而是任何"AI 建议、人确认"的模式（ChatGPT 对话、Cursor 补全都是 Copilot 范式）。两者的本质区别是自主程度：Copilot 每步都需要人确认，Agent 自主规划执行多步，人只在关键节点介入。
...
**Voyager 是由 NVIDIA 联合 Caltech、UT Austin、Stanford 等机构在 2023 年发布的里程碑式 Agent，在 Minecraft 中实现了"终身学习"——Agent 能自主探索、发现新技能、存储技能库，并在未来的任务中复用之前学到的技能。**
