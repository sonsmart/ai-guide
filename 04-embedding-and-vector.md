...
**Embedding = 把任何形式的数据（文本、图片、音频、视频）转成向量，让机器能计算它们之间的相似度。**

文本 Embedding 让机器理解语义相似性（"提高代码质量"和"编写高质量软件"是一回事）。多模态 Embedding 更进一步，把不同模态的数据映射到同一个向量空间，让文字和图片也能比较相似度——这是"用文字搜图片"的技术基础。
...
| **GTE-Qwen2-7B** (Alibaba) | 3584 | 32768 | ✅ 优 | ✅ 优 | ✅ | ~70.2 | 免费 | ✅ |
...
| **分布式** | ✅ 原生 | ✅ 支持分片/集群 | ✅ 托管 | ⚠️ 需PG方案 | ❌ | ❌ | ✅ 原生 |
| **持久化** | ✅ | ✅ | ✅ | ✅ | ✅ | ⚠️ 手动 | ✅ |
| **多租户** | ✅ partition | ✅ collection | ✅ namespace | ✅ schema | ❌ | ❌ | ✅ tenant |
| **混合搜索** | ✅ | ✅ | ✅ | ✅ (tsvector) | ❌ | ❌ | ✅ BM25 |
| **最大向量数** | 数十亿 | 数亿 | 数十亿 | 数千万 | 数百万 | 数十亿 | 数亿 |
| **社区** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ |
| **价格** | 开源免费 | 开源免费 | 免费/$20起（Builder） | 免费(PG扩展) | 开源免费 | 免费(MIT) | 开源免费 |

**性能对比（数量级参考，具体数字因硬件/数据/配置差异较大，以官方 benchmark 为准）：**

| 指标 | Milvus | Qdrant | Pinecone | pgvector | Chroma | FAISS |
|------|--------|--------|----------|----------|--------|-------|
| 查询延迟 | 低 | 极低 | 低（网络延迟+） | 中 | 中 | 极低（纯内存） |
| 内存占用 | 中 | 低 | N/A | 高 | 中 | 低 |
| Recall@10 | 高 | 高 | 高 | 高（HNSW）/中（IVFFlat） | 高 | 高 |

> 注意：性能受索引类型、数据规模、硬件配置影响极大，选型前建议在自己的数据集上实测。FAISS 延迟最低但不提供持久化，pgvector HNSW 模式 recall 约 0.99，IVFFlat 约 0.92-0.95。
...
Milvus：
  ✅ 原生分布式，支持十亿级向量
  ✅ Zilliz Cloud 托管服务
  ❌ 部署复杂（standalone依赖etcd、MinIO；cluster模式另需Pulsar/Kafka）
  → 适合：大规模生产环境、企业级
...
| 内存占用 | ⭐ (大) | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ (小) | ⭐⭐⭐ |
| 建索引速度 | ⭐⭐ (慢) | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| 支持增量插入 | ✅ | ⚠️ 可增量（聚类中心固定，精度随数据分布变化下降，建议定期重建） | ❌ 需重建 | ✅ |
...
| **SigLIP** (Google) | 文本+图片 | 768/1024/1152（Base/Large/So400m） | CLIP 改进版，Sigmoid Loss |
...
**选型考虑的四个核心维度：**
- **数据量**：原型/小项目用 Chroma/pgvector（上手简单），中大规模用 Qdrant/Milvus（实际都支持亿级），超大规模用 Milvus 集群/Pinecone Enterprise（注意：具体阈值不绝对，以实测性能为准）
- **部署方式**：本地/私有化用开源（Qdrant/Milvus），不想运维用云托管（Pinecone/Qdrant Cloud）
- **已有技术栈**：用了 PostgreSQL 可直接加 pgvector 插件，零额外基础设施
- **成本**：开源免费（Qdrant/Milvus/Chroma）vs 付费托管（Pinecone $20/月起）
