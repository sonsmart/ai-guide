...
│   └── Word2Vec → ELMo → Attention → Transformer 的进化链
└── A2.6 深度学习框架
...
| **3. ELMo (2018)** | 双向 LSTM | 上下文相关的词向量 | 仍是 RNN 架构，串行瓶颈 |
...
Word2Vec: 静态 embedding（一个词一个向量，与上下文无关）
    ↓ 问题：多义词怎么办？
ELMo: 动态 embedding（根据上下文变化），但用 LSTM，串行
    ↓ 问题：怎么并行 + 长距离？
Attention: 不再按顺序处理，而是让每个词直接"看到"所有其他词
    ↓ 组合起来
Transformer: Self-Attention + FFN + 位置编码，完全替代 RNN
```
